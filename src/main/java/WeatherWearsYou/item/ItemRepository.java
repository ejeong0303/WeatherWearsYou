...
public interface ItemRepository extends JpaRepository<Item, String>, JpaSpecificationExecutor<Item> {
    ...
    List<Item> getItems(Integer category, String type, Integer gen);
    ...
    @Query(value = "SELECT * FROM items WHERE category = ?1 and item_type LIKE ?2 and price > ?3 and price < ?4  and gender = ?5 order by item_rank LIMIT 4", nativeQuery = true)
    List<Item> getItemsByPrice(Integer category, String type, Integer min, Integer max, Integer gen);
